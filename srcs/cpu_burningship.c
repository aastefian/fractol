...
/*   Updated: 2016/09/28 12:54:06 by svilau           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_com c)
{
	...
	{
		zn_r = fabs((zn_1r * zn_1r) - (zn_1i * zn_1i)) + c.r;
		zn_i = 2 * fabs(zn_1r * zn_1i) + c.i;
		zn_1r = zn_r;
		zn_1i = zn_i;
...
void			cpu_bs(t_data_bs *data, unsigned int **a_h)
{
	...
	while (row < DEF_WIN_HEIGHT)
	{
		...
		while (col < DEF_WIN_WIDTH)
		{
			...
			a_d[index] = coordinate_iterations_bs(data,
					const_bs(data, row, col));
			col++;
		}
		row++;
	}
