...
/*   Updated: 2016/09/30 10:30:28 by svilau           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"
#include "../cudaheader/cuda_bs.h"
...
void			bs_pixel_color(t_data_bs *data, t_2d_p *pixel,
							int iteration)
{
	...
	while (i < DEF_WIN_HEIGHT)
	{
		...
		while (j < DEF_WIN_WIDTH)
		{
			...
			if ((a_h[i * DEF_WIN_WIDTH + j] < DEF_GPU_MAX_ITERATIONS &&
				...
				bs_pixel_color(data, &pixel, a_h[i * DEF_WIN_WIDTH + j]);
			else
				pixel.color = 0;
