...
__global__ void bs_2(unsigned int *a, unsigned int constw, unsigned int consth, float middlex, float middley, float scale, unsigned int max)
{
  ...
  while (iteration < max)
  {
    zn_r = fabs((zn_1r * zn_1r) - (zn_1i * zn_1i)) + cr;
	zn_i = 2 * fabs(zn_1r * zn_1i) + ci;
	zn_1r = zn_r;
	zn_1i = zn_i;
...
extern "C" void cuda_call_bs(unsigned int *a_h, unsigned int constw, unsigned int consth, float middlex, float middley, float scale, unsigned int max, unsigned int reset)
{
  ...
  if (!reset)
  {
   ...
   bs_2 <<< grid_size, block_size, 0 >>> (a_d, constw, consth, middlex, middley, scale, max);
